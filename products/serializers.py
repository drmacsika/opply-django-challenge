...
class ProductSerializer(serializers.ModelSerializer):
    ...
    def validate_id(self, value):
        try:
            ...
            raise serializers.ValidationError(f"Product with the id '{value}' does not exist")
        return value
    ...
    def validate(self, data):
        ...
        if product.is_out_of_stock:
            raise serializers.ValidationError(f"Product with the id '{product_id}' is out of stock.")
        if product.insufficient_quantity(quantity):
            raise serializers.ValidationError(f"Not enough products in stock. Only {product.quantity} left.")
        return data
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def validate_products(self, value):
        ...
        if len(set(ids[0::2])) != len(ids[0::2]):
            raise serializers.ValidationError("One or more product ids are repeated. Please specify one product id per item.")
        return value
    ...
    def create(self, validated_data):
        ...
        if customer.get("username") != authenticated_username:
            raise serializers.ValidationError("Customer username is not the logged in user. Please login with the customer username.")
...
class CustomerOrderHistorySerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        data["products_ordered"] = [{"name": p.name, "price": p.price, "quantity": p.quantity} for p in instance.products.all()]
        data.pop("products")
        return data
